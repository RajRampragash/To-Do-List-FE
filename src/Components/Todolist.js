...
function Todolist(props) {
    const todolist = props.todolist.map((task,index) => {
        const taskComplete = task => {
            axios.put(`http://localhost:8080/api/tasks/${task._id}` , {
                _id : task._id,
                todo: task.todo,
        ...
        const removeTask = id => {
            axios.delete(`http://localhost:8080/api/tasks/${id}`).then(res => props.removeTask(res.data)).catch(err => console.log(err))
        } 
        return <li key = {index}>
